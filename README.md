...
Two ways to run the program are provided.
 1. (Recommended) Use the [PACIAE.sh](PACIAE.sh) shell-script to compile, link and build the program, generate input files and run the program automatically.
 ...
 2. Manual running. Use the **`make`** tool to compile the source code, link and build the PACIAE program. Then execute it by hand. **`GFortran`** and **`g++`** compilers have been pre-specified inside **Makefile**:
...
- In "main_40.f90", "Pythia8CppInterface.hpp" and "Pythia8CppInterface.cpp", the random 2*pi angle of the impact parameter is introduced for the Angantyr modes.
