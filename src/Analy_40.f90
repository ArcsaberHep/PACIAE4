...
!! Author: Ben-Hao Sa, December 2019 - August 2025.
...
!       Largest momentum allowed for particles.
        dpmax = adj1(27)
...
!       Excludes the projectile and the target spectator nucleons.
            if( ( iKF == 2212 .OR. iKF == 2112 ) .AND. ppt <= 1D-5 ) cycle

!       Uses the 1st set of cuts for +- multiplicities.
            i_set = 1
!       Statistic of the negative multiplicity.
            if( charge < 0D0 )then
                nminf = nminf + w
                do i = 1, iflmax, 1
                    if( c(i) < afl( i_set, i, 1 ) &
                        .OR. c(i) > afl( i_set, i, 2 ) ) goto 700
                end do
                nmin = nmin + w
700         end if
!       Statistic of the positive multiplicity.
            if( charge > 0D0 )then
                nchaf = nchaf + w
                do i = 1, iflmax, 1
                    if( c(i) < afl( i_set, i, 1 ) &
                        .OR. c(i) > afl( i_set, i, 2 ) ) goto 701
                end do
                ncha = ncha + w
701         end if
...
!       For total, charged and pi+K+p.
            call stati_particle( yy, ppt, eta, p5, charge, iKF, w )
            call flow_particle( p1, p2, yy, ppt, eta, charge, iKF, w )

!       For particles the user specified in "usu.dat".
            do kk = 1, ispmax, 1
                kf = ispkf(kk)
                if( iKF /= kf ) cycle
                call stati_h( yy, ppt, eta, p5, iKF, kk, w, an, anf, bn, bnf )
                call flow_hadron( p1, p2, yy, ppt, eta, kk, w )
                exit
            end do
        end do

!       Statistics of multiplicity distributions and mean pT <pT>.
!       The 5-th distribution is the particle multiplicity distribution.
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        subroutine stati_h( y, pt, eta, p5, iKF, kk, ww, a, af, b, bf )
...
!       Displays the subprocess status.
        ...
        case(2)
            NAME_SUB = "Drell-Yan"
        case(3)
            NAME_SUB = "J/psi production (color singlet via NRQCD)"
        case(4)
            NAME_SUB = "c-cbar production"
        case(5)
            NAME_SUB = "prompt photon"
        case(6)
            NAME_SUB = "soft QCD"
        case(7)
            NAME_SUB = "single W+/- production"
        case(8)
            NAME_SUB = "hard QCD"
        case(9)
            NAME_SUB = "single Z0 production"
        case(10)
!           For hh/hA/AB, inelastic non-diffractive (minimum-bias, MB)
            NAME_SUB = "inelastic, non-diffractive (minimum-bias, MB)"
!           For lh/lA, deep inela. scatterings (DIS). t-channel boson exchange.
            if( ( IS_LEPTON( KF_proj ) .AND. IS_HADRON( KF_targ )  ) &
                ...
                NAME_SUB = "deep inelastic scatterings (DIS)"
!           For ll.
            else if( IS_LEPTON( KF_proj ) .AND. IS_LEPTON( KF_targ ) )then
                if( KF_proj + KF_targ == 0 )then
                    NAME_SUB = "l+ + l- -> gamma*/Z -> q + qbar"
                else
                    NAME_SUB = "l1 + l2bar -> W+- -> q1 + q2bar."
                end if
            end if
        case default
            ...
            if( INT(adj1(5))  ==  0 )then
                ...
                if( .NOT.IS_PYTHIA8(i_mode) )then
                    ...
                    if( INT(adj1(5)) == -1 )then
                        ...
                        NAME_SHAD = "preset 208Pb EPS09 LO nPDF"
                    end if
                end if
            end if
        end if
