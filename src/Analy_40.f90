...
!       the 5-th distribution is particle multiplicity distribution
            if(idf < 1 .or. idf > n_bin_hist .OR.bnf(kkk,1) < 1D-10) &
                goto 405
            anf(idf,5,kkk) = anf(idf,5,kkk) + w/asd(5)
!       activate the window
405         continue
            idd = INT( bn(kkk,1) / w / asd(5) + 1 )
            if(idd < 1 .or. idd > n_bin_hist .OR. bn(kkk,1) < 1D-10) &
                goto 404
...
!       the 7-th distribution is mean pT VS. particle multiplicity distribution
            if( idf >= 1 .AND. idf <= n_bin_hist .AND. &
                bnf(kkk,1) > 0D0 )then
                dMean_pT_f = bnf(kkk,2) / bnf(kkk,1)
                anf(idf,7,kkk) = anf(idf,7,kkk) + dMean_pT_f*w !/asd(7)
            end if
            idd = INT( bn(kkk,1) / w / asd(7) + 1 )
            if( idd >= 1 .AND. idd <= n_bin_hist .AND. &
                bn(kkk,1) > 0D0 )then
                dMean_pT = bn(kkk,2) / bn(kkk,1)
                an(idd,7,kkk) = an(idd,7,kkk) + dMean_pT*w !/asd(7)
            end if
        enddo
...
!       the 7-th distribution is mean pT VS. particle multiplicity distribution
            if( idf >= 1 .AND. idf <= n_bin_hist .AND. &
                dMult_f(kkk,1) > 0D0 )then
                dMean_pT_f = dMult_f(kkk,2) / dMult_f(kkk,1)
                distr_f(idf,7,kkk) = distr_f(idf,7,kkk) + dMean_pT_f*w !/asd(7)
            end if
            idd = INT( dMult(kkk,1) / w / asd(7) + 1 )
            if( idd >= 1 .AND. idd <= n_bin_hist .AND. &
                dMult(kkk,1) > 0D0 )then
                dMean_pT = dMult(kkk,2) / dMult(kkk,1)
                distr(idd,7,kkk) = distr(idd,7,kkk) + dMean_pT*w !/asd(7)
            end if
        end do
...
!!      Online statistics for NA, AN, and AA collisions.
        IMPLICIT DOUBLE PRECISION(A-H, O-Z)
        IMPLICIT INTEGER(I-N)
        common/sa1/kjp21,non1,bp,iii,neve,nout,nosc
        common/sa7/ispmax,isdmax,iflmax,ispkf(100),n_bin_hist,asd(10), &
...
!           Analyses for pp,pA and AB
            if( kk > 0 ) call stati_parton( yy, ppt, eta, p5, kf, kk, w )
        end do
...
!           The 7-th distribution is mean pT VS. particle multi. distribution
            if( idf >= 1 .AND. idf <= n_bin_hist .AND. &
                bn_p_f(kkk,1) > 0D0 )then
                dMean_pT_f = bn_p_f(kkk,2) / bn_p_f(kkk,1)
                an_p_f(idf,7,kkk) = an_p_f(idf,7,kkk) + dMean_pT_f*w !/asd(7)
            end if
            idd = INT( bn_p(kkk,1) / w /asd(7) + 1 )
            if( idd >= 1 .AND. idd <= n_bin_hist .AND. &
                bn_p(kkk,1) > 0D0 )then
                dMean_pT = bn_p(kkk,2) / bn_p(kkk,1)
                an_p(idd,7,kkk) = an_p(idd,7,kkk) + dMean_pT*w !/asd(7)
            end if
        end do
...
!       Displays the nuclear shadowing status.
        ...
        write(10,*) "#! Nulear shadowing / nPDF : " // &
...
!!      Outpouts seven distributions of hadrons.
        ...
        common/sa7/ispmax,isdmax,iflmax,ispkf(100),n_bin_hist,asd(10), &
         afl(100,10,2),i_y_or_eta
        dimension sao(100,10,100),  sbo(100,2), &
...
!        and ispmax particles specified in usu.dat.
        ...
        write(10,*) "#! particle mean pT <pT>, partial (" // &
                    cut_off(1,1)//" < pT < "//cut_off(2,1)//";"// &
                    cut_off(1,2)//" <"//cyeta//"< "//cut_off(2,2)//") ="
        write(10,*) (sbo_mult(ll,2),ll=1,3,1), (sbo(ll,2),ll=1,ispmax,1)
        write(10,*) "#! particle mean pT <pT>, full ="
        write(10,*) (sbo_mult_f(ll,2),ll=1,3,1),(sbof(ll,2),ll=1,ispmax)
...
!       Outputs particle multiplicities and mean pT of partons.
        ...
        write(10,*) "#! particle mean pT <pT>, partial (" // &
                    cut_off(1,1)//" < pT < "//cut_off(2,1)//";"// &
                    cut_off(1,2)//" <"//cyeta//"< "//cut_off(2,2)//") ="
        write(10,*) (sbn_p(ll,2)/swt,ll=1,15,1)
        write(10,*) "#! particle mean pT <pT>, full ="
        write(10,*) (sbn_p_f(ll,2)/swt,ll=1,15,1)
