...
!Lei20230819    integer:: i, j, k, l, m, n
        implicit real(kind=8) (a-h, o-z)   !Lei20230821
        implicit integer (i-n)   !Lei20230821
...
!Lei20240622E---
        character(len=800) :: comment_line(800)
...
!Lei20240214E---
!Lei20240916B---
        real(kind=8) :: sum_bParam_ANG(3), sum_sigma_ANG(4), sum_Ncoll_ANG(8), sum_Npart_ANG(4,2)
        real(kind=8) :: nCollNDTot, nCollND, nCollSDP, nCollSDT, nCollDD, nCollCD, nCollEL, nCollTot
...
! Reads out input parameters from the "input_rms_analysis.dat".
        ...
        ! write(*,*) "Please input number of files/run/CPUs (n_run) using keyborad then press Enter:"
        ! read(*,*) n_file
        read(98,*) n_file, n_event_per_file   !Lei20230820
        read(98,*) i_sim_mode, i_b_sampling_method   !Lei20230819
        read(98,*) n_particle_specie, n_distribution, n_kine_cut, n_bin_hist   !Lei20240215
        close(98)
...
!Lei20240214B-------------------------------------------------------------------
        ! Cycle over all of the "../PACIAE_i/rms.out" files.
...
!Lei20230819B----------
            ...
            i_line = 0   !Lei20240214
            ! #!***************************************************************************!#
            ! #!*********************|    PACIAE  Analysis  Output    |********************!#
            ! #!***************************************************************************!#
            !
            ! #! Now is   HH:MM:SS   DD/MM/YYYY
            ! #! Seed (PACIAE default=900000000) = *********
            ! #! Collision system        : ***
            ! #! Simulation mode: ***
            ! #! Nulear shadowing: ***
            ! #! Tune: ***
            ...
            do j=1,16,1
                i_line = i_line + 1
                read(99,"(A800)") comment_line( i_line )
            end do
            ! #!-----------------------------------------------------------------------------
            ...
            ! #! sigmaTot, sigmaTotErr, sigmaND, sigmaNDErr =
            i_line = i_line + 1
            read(99,"(A800)") comment_line( i_line )
            ! Data.
            read(99,*) sigmaTot, sigmaTotErr, sigmaND, sigmaNDErr
                sum_sigma_ANG(1) = sum_sigma_ANG(1) + sigmaTot
                sum_sigma_ANG(2) = sum_sigma_ANG(2) + sigmaTotErr
                sum_sigma_ANG(3) = sum_sigma_ANG(3) + sigmaND
                sum_sigma_ANG(4) = sum_sigma_ANG(4) + sigmaNDErr
            ! #! nCollNDTot, nCollND, nCollSDP, nCollSDT =
            i_line = i_line + 1
            read(99,"(A800)") comment_line( i_line )
            ! Data.
            read(99,*) nCollNDTot, nCollND, nCollSDP, nCollSDT
                sum_Ncoll_ANG(1) = sum_Ncoll_ANG(1) + nCollNDTot
...
!==============================================================================!
            ...
            ! read(99,*) shebang_sign, ( name_specie(j), j=1,n_particle_specie,1 )
            i_line = i_line + 1
            read(99,"(A800)") comment_line( i_line )
            ! Data.
...
!==============================================================================!
            ...
            ! #! g      u+d+s + anti- ... u, d, s, ...
            ! read(99,*) shebang_sign, ( name_parton(j), j=1,n_parton_specie,1 )
            i_line = i_line + 1
            read(99,"(A800)") comment_line( i_line )
            ! Data.
...
!Lei20230820 Event-(file-) averaging.
        ...
        sum_mean_pT_specie_partial = sum_mean_pT_specie_partial / n_file
        sum_mean_pT_specie_full = sum_mean_pT_specie_full / n_file
        ...
        sum_mean_pT_parton_partial = sum_mean_pT_parton_partial / n_file
        sum_mean_pT_parton_full = sum_mean_pT_parton_full / n_file
...
! #! Seed (PACIAE default=900000000) = *********
        i_line = i_line + 1
        ! write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
        write(100,*) "#! Seed (PACIAE default=900000000) = *********"
! #! Collision system: ***
! #! Simulation mode: ***
! #! Nulear shadowing: ***
! #! Tune: ***
...
! #! nCollDD, nCollCD, nCollEL, nCollTot =
        i_line = i_line + 1
        write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
        write(100,*) ( sum_Ncoll_ANG(i), i=5,8,1 )
! #! nPartProj, nAbsProj, nDiffProj, nElProj =
        i_line = i_line + 1
        write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
...
! #! tatol charged  pi+K+p name_1 ... name_20
        ! write(100,*) "#! ", ( name_specie(j), j=1,n_particle_specie,1 )
        i_line = i_line + 1
        write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
! Data.
...
!==============================================================================!
        ...
        loop_hadron_output: do i_distr=1,n_distribution,1
            ...
            ! #! x    total  charged  pi+K+p  name_1 ... name_20
            ! write(100,*) "#!       " // id_abscissa(i_distr) // "               ", (name_specie(j),j=1,n_particle_specie,1)
            i_line = i_line + 1
            write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
...
!==============================================================================!
        ...
        ! #! g      u+d+s + anti- ... u, d, s, ...
        ! write(100,*) "#! ", ( name_parton(j), j=1,n_parton_specie,1 )
        i_line = i_line + 1
        write(100,*) TRIM(ADJUSTL( comment_line( i_line ) ))
        ! Data.
...
!==============================================================================!
        ...
        loop_parton_output: do i_distr=1,n_distribution,1
            ...
            ! #! x    g     u+d+s + anti-   u, d, s, ...
            ! write(100,*) "#!       " // id_abscissa(i_distr) // "               ", (name_parton(j),j=1,n_parton_specie,1)
            i_line = i_line + 1
            write(100,*) TRIM(ADJUSTL( comment_line(i_line) ))
            ! Outputs data of partial phase-space.
            do i_bin=1,n_bin_hist,1
